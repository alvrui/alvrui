...
interface SelectedElementsListProps {
  ...
  getProblemasForElement: (elementId: string) => ProblemaValidacion[];
...
const DraggableElement: React.FC<DraggableElementProps> = ({
  ...
  // Get available roles for this story element
  const availableRoles = storyElement.role_in_story.length > 0 
    ? storyElement.role_in_story
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center space-x-2">
            ...
            {hasProblems && (
              <span className="text-xs px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-200 rounded">
                !
              </span>
            )}
          </div>
...
const SelectedElementsList: React.FC<SelectedElementsListProps> = ({
  trama,
  storyElements,
  personajes,
  onRemoveElement,
    ...
    return (
      <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 text-center">
        ...
        <p className="text-sm text-gray-400 dark:text-gray-500">
          Busca y selecciona elementos del catalogo para construir tu trama
        </p>
      </div>
    );
  }
        ...
        {trama.story_elements.map((element, index) => {
          ...
          return (
            <DraggableElement
              ...
              onAssignPersonaje={(role, personajeId) => onAssignPersonaje(element.id, role, personajeId)}
              problemas={problemas}
